            ...
            <?php else: ?>
                <?php foreach ($applications as $app): ?>
                    <div class="card">
                        ...
                        <?php if ($tab === 'pending'): ?>
                            <div class="card-actions" style="flex-direction:column;align-items:stretch;gap:.75rem">
                                <form method="post" action="<?= url('app/actions/loan_review.php'); ?>"><?= csrf_field('app'); ?>
                                    <input type="hidden" name="application_id" value="<?= (int)$app['id']; ?>">
                                    <input type="hidden" name="action" value="approve">
                                    <button class="btn btn-primary btn-sm" type="submit">Approve</button>
                                </form>
                                <form method="post" action="<?= url('app/actions/loan_review.php'); ?>" style="display:flex;gap:.5rem;align-items:center"><?= csrf_field('app'); ?>
                                    <input type="hidden" name="application_id" value="<?= (int)$app['id']; ?>">
                                    <input type="hidden" name="action" value="reject">
                                    <input type="text" name="rejection_reason" placeholder="Reason for rejection" style="flex:1;padding:6px 10px;border:1px solid #ddd;border-radius:8px">
                        ...
                        <?php elseif ($tab === 'rejected'): ?>
                            <div class="card-actions" style="flex-direction:column;align-items:stretch;gap:.5rem">
                                <span class="badge-status badge-danger">Rejected</span>
                                <div><strong>Reason:</strong> <?= htmlspecialchars($app['rejection_reason'] ?? 'No reason recorded'); ?></div>
                            </div>
                        <?php else: ?>
