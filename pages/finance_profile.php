...
$profStmt = $pdo->prepare('SELECT institution, designation FROM finance_profiles WHERE user_id = ?');
...
<div class="dashboard-layout">
    <?php $active = 'settings'; require __DIR__ . '/../app/includes/finance_sidebar.php'; ?>

    <main class="main-content">
        <div class="dash-header">
            <div>
                <h1>My Profile</h1>
                <p>View and update your account, institution, and payment details.</p>
            </div>
        ...
        <div class="card dashboard-section" style="margin-bottom:1.5rem">
            <h2 style="font-size:1.15rem;margin-bottom:1.25rem"><i class="fas fa-building-columns" style="color:var(--emerald);margin-right:8px"></i> Institution Details</h2>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.25rem">
                <div><p class="muted">Institution</p><p style="font-weight:600"><?= htmlspecialchars($profile['institution'] ?? 'Not set'); ?></p></div>
                <div><p class="muted">Designation</p><p style="font-weight:600"><?= htmlspecialchars($profile['designation'] ?? 'Not set'); ?></p></div>
            </div>
        </div>
        ...
        <section class="card dashboard-section" id="profile" style="margin-bottom:1.5rem">
            ...
            <form method="post" action="<?= url('app/actions/settings_update.php'); ?>"><?= csrf_field('app'); ?>
                ...
                <input type="hidden" name="return_to" value="pages/finance_profile.php">
                ...
                <h3 class="form-section-title">Institution</h3>
                <div class="form-row">
                    <div class="form-group"><label>Institution</label><input type="text" name="extra" value="<?= htmlspecialchars($profile['institution'] ?? ''); ?>"></div>
                    <div class="form-group"><label>Designation</label><input type="text" name="address" value="<?= htmlspecialchars($profile['designation'] ?? ''); ?>"></div>
                </div>
