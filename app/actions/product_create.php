...
require_login();
require_csrf_token($_POST['csrf_token'] ?? null, 'product_create', 'pages/manage_products.php?product_error=invalid#add');

$user = current_user();
if (!$user || $user['role'] !== 'farmer') {
    redirect('pages/manage_products.php?product_error=role');
}
...
if ($name === '' || $categoryId <= 0 || $price <= 0 || $quantity <= 0 || !in_array($unit, $allowedUnits, true)) {
    redirect('pages/manage_products.php?product_error=invalid#add');
}
...
if (!$categoryStmt->fetch()) {
    redirect('pages/manage_products.php?product_error=category#add');
}

$imagePath = null;
if (!empty($_FILES['product_image']) && ($_FILES['product_image']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
    if ($_FILES['product_image']['error'] !== UPLOAD_ERR_OK) {
        redirect('pages/manage_products.php?product_error=image#add');
    }

    if ((int)$_FILES['product_image']['size'] > 3 * 1024 * 1024) {
        redirect('pages/manage_products.php?product_error=image_size#add');
    }

    $extension = strtolower(pathinfo($_FILES['product_image']['name'], PATHINFO_EXTENSION));
    $allowedExtensions = ['jpg', 'jpeg', 'png', 'webp'];
    if (!in_array($extension, $allowedExtensions, true)) {
        redirect('pages/manage_products.php?product_error=image_type#add');
    }
    ...
    if (!move_uploaded_file($_FILES['product_image']['tmp_name'], $target)) {
        redirect('pages/manage_products.php?product_error=image_save#add');
    }
...
try {
    ...
    $pdo->rollBack();
    redirect('pages/manage_products.php?product_error=save#add');
}

redirect('pages/manage_products.php?product_created=1#inventory');
