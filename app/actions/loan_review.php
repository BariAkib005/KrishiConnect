...
require_csrf_token($_POST['csrf_token'] ?? null, 'app', 'pages/loan-management.php?error=csrf');
...
if ($action === 'reject') {
    ...
    $upd = $pdo->prepare('UPDATE loan_applications SET status = "rejected", reviewed_by = ?, rejection_reason = ? WHERE id = ?');
    $upd->execute([(int)$user['id'], $reason, $applicationId]);
